...
esp_err_t gsl3680_touch_init_local(esp_lcd_panel_io_handle_t io, const esp_lcd_touch_config_t *config, esp_lcd_touch_handle_t *out_touch)
{
     esp_err_t ret = ESP_OK;

    ESP_LOGI(TAG, "gsl3680_touch_init_local called, io=%p", io);

    assert(io != NULL);
    assert(config != NULL);
    assert(out_touch != NULL);

    ESP_LOGI(TAG, "config: x_max=%d, y_max=%d, rst=%d, int=%d",
             config->x_max, config->y_max, config->rst_gpio_num, config->int_gpio_num);
    ...
    /* Quick I2C test before full init */
    ESP_LOGI(TAG, "Testing I2C communication...");
    uint8_t test_buf[4] = {0};
    esp_err_t test_ret = touch_gsl3680_i2c_read(esp_lcd_touch_gsl3680, 0xf0, test_buf, 4);
    ESP_LOGI(TAG, "I2C test read 0xf0: ret=%d, data=%02x %02x %02x %02x",
             test_ret, test_buf[0], test_buf[1], test_buf[2], test_buf[3]);
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "WARNING: Firmware verification failed! Touch may not work.");
    } else {
        ESP_LOGI(TAG, "Firmware verified OK (0x5a5a5a5a)");
    }
    // touch_gsl3680_read_cfg(esp_lcd_touch_gsl3680);
...
static uint32_t read_data_call_count = 0;
...
// #endif
    ...
    if (read_data_call_count <= 5 || (read_data_call_count % 100 == 0)) {
        ESP_LOGI(TAG, "read_data[%lu]: err=%d, raw[0-7]=%02x %02x %02x %02x %02x %02x %02x %02x",
                 read_data_call_count, err,
...
static esp_err_t touch_gsl3680_i2c_read(esp_lcd_touch_handle_t tp, uint16_t reg, uint8_t *data, uint8_t len)
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "I2C read reg 0x%02x failed: %d", reg, ret);
    }
    return ret;
}
...
static esp_err_t touch_gsl3680_i2c_write(esp_lcd_touch_handle_t tp, uint16_t reg, uint8_t *data,uint8_t len)
{
    ...
    esp_err_t ret = esp_lcd_panel_io_tx_param(tp->io, reg, data, len);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "I2C write reg 0x%02x failed: %d", reg, ret);
    }
    return ret;
}
...
static esp_err_t esp_lcd_touch_gsl3680_load_fw(esp_lcd_touch_handle_t tp)
{
    ...
    ESP_LOGI(TAG,"start load fw: %d entries", source_len);
    ...
    for(source_line=0;source_line<source_len;source_line++)
    {
        ...
        if (ret != ESP_OK) {
            write_errors++;
            if (write_errors <= 5) {
                ESP_LOGE(TAG, "FW write error at line %d, addr 0x%02x", source_line, addr);
            }
        }

        // Progress indicator every 1000 entries
        if (source_line > 0 && source_line % 1000 == 0) {
            ESP_LOGI(TAG, "FW load progress: %d/%d", source_line, source_len);
        }
    }

    if (write_errors > 0) {
        ESP_LOGE(TAG, "FW load completed with %d errors!", write_errors);
    } else {
        ESP_LOGI(TAG, "load fw success: %d entries written", source_len);
    }
    return ESP_OK;
}
